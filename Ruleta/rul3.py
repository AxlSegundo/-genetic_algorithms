...
# Definir el algoritmo genético con restricción
# Definir el algoritmo genético con restricción
def algoritmo_genetico(funcion_simb, variables, intervalos, nombre_funcion, restricciones=None, num_individuos=10, num_generaciones=1500, porcentaje_mutacion=0.1):
    ...
    with open(f'Ruleta/{nombre_funcion}_resultados.txt', 'w') as f:
...
for i, (funcion, vars, inter, restr) in enumerate(zip(funciones_simb, variables, intervalos, restricciones)):
    ...
    mejor_individuo, mejor_fitness = algoritmo_genetico(funcion, vars, inter, f'funcion_{i+1}', restricciones=restr)
